...
def templatematchtax(fname,tname):
    ...
    for pt in zip(*loc[::-1]):
        ...
        new_img3=img[pt[1]-20:pt[1] + h +80,1:width]
        cv2.imwrite(str(fname).replace(".png",tname + ".png"),new_img3)
        #further check
...
def imagecontours(imgth):
    ...
    for c in contours:
        ...
        if w>20 and h>60 and h<130 and (y2-y) > 3:
            ...
            cv2.imwrite("test" + str(x) + " " +str(y) + ".png",new_img3)
        y2 = y
...
def templatematch(fname,tname):
    ...
    for pt in zip(*loc[::-1]):
        ...
        height, width = img.shape
        #new_img3=img[pt[1]:pt[1] + h +20,pt[0]:pt[0] + w+80]
        new_img3=img[pt[1]-20:pt[1] + h +40,1:width + 300]
...
def createbat(fname):
    f = open('GS2.bat','w')
    f.write("echo Reading pdf \n")
    f.write("@echo off \n")
    for x in range(33):
        f.write('"C:\Temp\Dass\Software\gs9.19\\bin\gswin64c.exe" -dNOPAUSE -dBATCH -dFirstPage=' + str(x + 1) + ' -dLastPage=' + str(x+1) + ' -sDEVICE=png16m -r200x200 -sOutputFile="DInputPage' + str(x+1) + '.png" ' + fname + '\n')
    f.close()

def mainprocess():
    #print(
    createbat('DOCUMENT2.pdf')
    removefile()
    p = Popen("GS2.bat")
    ...
    for item in os.listdir():
        if item.endswith('r.png'):
            templatematchtax(item,'CountryOfRegistration.png')
            templatematchtax(item,'EntityClassification.png')
            templatematchtax(item,'RegistrationNumber.png')
            templatematchtax(item,'TaxResidencies.png')
